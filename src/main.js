...
/* ================================================================
   GAME ENGINE API — the hooks the crawl layer drives the demo with
   ================================================================ */
const ENGINE = {
  scene,
  cam,
...
/* a left "click" (no real drag) orders the party to that spot on the floor */
const _clickNdc = new THREE.Vector2();
const _clickRay = new THREE.Raycaster();
const _clickPlane = new THREE.Plane(new THREE.Vector3(0,1,0), 0);
const _clickPt = new THREE.Vector3();
cnv.addEventListener('pointerup', e=>{
  const wasClick = e.button===0 && !e.shiftKey &&
    Math.hypot(e.clientX-downX, e.clientY-downY) < 6;
  endDrag();
  if(!wasClick) return;
  const r = cnv.getBoundingClientRect();
  _clickNdc.set(((e.clientX-r.left)/r.width)*2-1, -((e.clientY-r.top)/r.height)*2+1);
  _clickRay.setFromCamera(_clickNdc, cam);
  if(_clickRay.ray.intersectPlane(_clickPlane, _clickPt))
    game.commandMove(_clickPt.x, _clickPt.z);
});
...
addEventListener('keydown', e=>{
  ...
  /* R/T (reforge) and H (heatmap) are disabled in game mode — they'd wreck a
     run in progress or fight the fog-of-war instance colors. */
  if(e.code==='KeyP'){ el.tPost.checked = !el.tPost.checked; POST.enabled = el.tPost.checked; }
...
/* -------- go -------- */
game.init(ENGINE);   // shows the party-setup overlay; embarking reforges into a run
forge(true);         // animated backdrop dungeon behind the setup screen
tick();
