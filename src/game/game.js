...
import {
  makeHero, normalizeHero, spawnMonster, CLASSES, MONSTER_THEMES, grantXp
} from './srd.js';
import {
...
class Game {
  ...
  nextDungeon(first = false) {
    ...
    for (const h of this.heroes) {
      ...
      h.data.rageUsed = false;
      h.data.smiteUsed = false;
      h.data.tidesUsed = false;
      h.raging = false;
      h.tempHp = 0;
      h.uncannyUsed = false;
      ...
      if (h.data.layOnHandsMax) h.data.layOnHands = h.data.layOnHandsMax;
  ...
  update(dt, elapsed) {
    ...
    if (this.combat) this.wasCombat = true;
      ...
      for (const h of alive) {
        const d = h.data;
        h._foughtThisCombat = false; // reset First Strike for next fight
        if (d.abilityUsed && d.abilityUsed.short) { d.abilityUsed.short = false; rested = true; }
        /* warlock pact slots + arcane recovery */
        if (d.classKey === 'warlock' && d.slotsMax) { d.slots = d.slotsMax; rested = true; }
        if (d.features && d.features.includes('arcaneRecovery') && d.slots < d.slotsMax) {
          d.slots = Math.min(d.slotsMax, d.slots + 1); rested = true;
        }
        d.smiteUsed = false;
        d.tidesUsed = false;
        h.uncannyUsed = false;
        /* song of rest: small heal */
        if (d.features && d.features.includes('songOfRest') && d.hp < d.maxHp) {
          d.hp = Math.min(d.maxHp, d.hp + 4 + d.level);
