...
const CHALLENGES = [
  ...
  /* ------- Intelligence: Religion ------- */
  {
    ...
    onSuccess: { text: 'You perform the purification ritual. The shrine glows with restored light — the party takes a short rest, fully healed!', reward: { kind: 'shortRest', fullHeal: true } },
  ...
  },
  {
    ...
    onSuccess: { text: 'You find a well-hidden nook. The party takes a short rest — short-rest abilities recharge.', reward: { kind: 'shortRest' } },
...
function precomputeReward(game, reward) {
  ...
  switch (reward.kind) {
    ...
    case 'shortRest': {
      out.data = { kind: 'shortRest', fullHeal: !!reward.fullHeal };
      out.detailText = reward.fullHeal
...
function applyComputedReward(game, computed, hero) {
  ...
  switch (d.kind) {
    ...
    case 'buff': {
      ...
      /* Survival "rested" boon also grants a short rest */
      if (challengeBuffType() === 'rested') {
